Mosaic.FrontLayerHolder = function (game, level, map) {
    ...
    this.fieldController = new Mosaic.FieldController(level);

    this.init(level);
};

Mosaic.FrontLayerHolder.prototype = {
    init: function (level) {
    ...
    drawTile: function (tile, listener) {
        var figure = new Mosaic.Figure(this.game, tile);
    ...
    inputUpListener: function () {
        ...
        this.fieldController.setFigureToField(
            this.layer.getTileX(this.marker.x - this.moveX),
            this.layer.getTileY(this.marker.y - this.moveY),
            this.currentFigure.tile);
